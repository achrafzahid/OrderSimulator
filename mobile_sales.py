"""
GlobalShop - Mobile App Sales Data Simulator
Generates daily CSV files with order data from mobile applications
Order IDs are prefixed with 'MOB-' to identify the mobile source
"""
...
# Configuration
ORDERS_PER_DAY = 150000  # Mobile apps generate 150k orders per day
NUM_DAYS = 5
START_DATE = datetime(2025, 4, 1)
OUTPUT_BASE_PATH = "globalshop-raw"  # Local simulation of GCS bucket

# Data generators
# Mobile apps more popular in certain countries
COUNTRIES = ['FR', 'DE', 'US', 'UK', 'ES', 'IT', 'JP', 'KR', 'AU', 'BR']
COUNTRY_WEIGHTS = [0.15, 0.15, 0.20, 0.12, 0.10, 0.08, 0.08, 0.05, 0.04, 0.03]
PRODUCT_IDS = [f'PROD-{i:05d}' for i in range(1, 1001)]  # 1000 different products
STATUS_OPTIONS = ['PAID', 'CANCELLED']
STATUS_WEIGHTS = [0.95, 0.05]  # Mobile has higher conversion: 95% PAID, 5% CANCELLED

def generate_order_id(day_counter, order_num):
    """Generate unique order ID with MOB prefix"""
    timestamp = int(datetime.now().timestamp() * 1000) % 1000000
    return f'MOB-{day_counter:03d}{order_num:06d}{timestamp:06d}'
...
def generate_order(day_counter, order_num, order_date):
    ...
    return {
        ...
        'quantity': random.randint(1, 8),  # Mobile users tend to buy smaller quantities
        'unit_price': round(random.uniform(5.99, 499.99), 2),  # Lower price range
...
def create_daily_csv(day_counter, order_date):
    ...
    output_file = output_dir / 'mobile_orders.csv'
    
    print(f"Generating {ORDERS_PER_DAY:,} mobile orders for {date_str}...")
...
def main():
    ...
    print("=" * 70)
    print("GlobalShop Mobile App Sales Data Generator")
    print("=" * 70)
    ...
    print("=" * 70)
    print(f"✓ COMPLETE: Generated {total_orders:,} total mobile orders")
