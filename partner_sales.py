"""
GlobalShop - Partner E-commerce Sales Data Simulator
Generates daily CSV files with order data from third-party partners
Order IDs are prefixed with 'PTR-' to identify the partner source
"""
...
# Configuration
ORDERS_PER_DAY = 150000  # Partner channels generate 150k orders per day
NUM_DAYS = 5
START_DATE = datetime(2025, 4, 1)
OUTPUT_BASE_PATH = "globalshop-raw"  # Local simulation of GCS bucket

# Data generators
# Partners operate in various markets
COUNTRIES = ['US', 'UK', 'DE', 'FR', 'CN', 'JP', 'IN', 'BR', 'MX', 'CA', 'AU', 'RU']
COUNTRY_WEIGHTS = [0.25, 0.15, 0.12, 0.10, 0.10, 0.08, 0.07, 0.05, 0.03, 0.02, 0.02, 0.01]
PRODUCT_IDS = [f'PROD-{i:05d}' for i in range(1, 1001)]  # 1000 different products
STATUS_OPTIONS = ['PAID', 'CANCELLED']
STATUS_WEIGHTS = [0.88, 0.12]  # Partners have lower conversion: 88% PAID, 12% CANCELLED

def generate_order_id(day_counter, order_num):
    """Generate unique order ID with PTR prefix"""
    timestamp = int(datetime.now().timestamp() * 1000) % 1000000
    return f'PTR-{day_counter:03d}{order_num:06d}{timestamp:06d}'
...
def generate_order(day_counter, order_num, order_date):
    ...
    return {
        ...
        'quantity': random.randint(1, 15),  # Partners may have bulk orders
        'unit_price': round(random.uniform(4.99, 1999.99), 2),  # Wider price range
...
def create_daily_csv(day_counter, order_date):
    ...
    output_file = output_dir / 'partner_orders.csv'
    
    print(f"Generating {ORDERS_PER_DAY:,} partner orders for {date_str}...")
...
def main():
    ...
    print("=" * 70)
    print("GlobalShop Partner E-commerce Sales Data Generator")
    print("=" * 70)
    ...
    print("=" * 70)
    print(f"✓ COMPLETE: Generated {total_orders:,} total partner orders")
